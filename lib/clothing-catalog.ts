...
export const getClothingCatalog = (): ClothingItem[] => {
  ...
  const autoGeneratedItems: ClothingItem[] = availableModels.map((modelUrl, index) => {
    ...
    return {
      ...
      position: category === 'shoes' ? [0, -2.5, 0] : 
                category === 'bottoms' ? [0, -2, 0] : 
                category === 'dresses' ? [0, -1.5, 0] : [0, -1, 0],
      scale: 1,
      physics: {
